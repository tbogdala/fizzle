...
The graphicsprovider module defines a common interface to the underlying
graphics 'driver' (e.g. OpenGL, OpenGL ES, etc...).

The fizzle library will use this so that the client application can
initialize the correct provider and then get access to that interface
from anywhere in the application.

This makes the assumtion that there will only be one graphicsprovider
initialized for any given application.

*/

package graphicsprovider
...
// Texture is a type indicating the uint32 use as an OpenGL texture
type Texture uint32

// Buffer is a type indicating the uint32 use as an OpenGL buffer
type Buffer uint32

// Program is a type indicating the uint32 use as an OpenGL shader program
type Program uint32
...
type GraphicsProvider interface {
	...
	BindFragDataLocation(p Program, color uint32, name string)
	...
	TexStorage3D(target Enum, level int32, intfmt uint32, width, height, depth int32)
...
// These enumeration constants were ripped right from the go-gl project's
// implementation of OpenGL 3.3.
