...
func main() {
	...
	mainWindow := initGraphics("Joystick Mapper", width, height)
	...
	createStateWidgets := func() {
		...
		if glfw.JoystickPresent(glfw.Joystick1) {
			...
			nameWidget = uiManager.CreateLabel(font, fizzle.UIAnchorTopMiddle, mgl.Vec3{0.0, 0.0, 10.0}, joystickName)
		} else {
			joystickName = "Joystick1 Not Detected"
			uiManager.CreateLabel(font, fizzle.UIAnchorTopMiddle, mgl.Vec3{0.0, 0.0, 10.0}, joystickName)
		...
		for i, b := range buttons {
			...
			w = uiManager.CreateLabel(font, fizzle.UIAnchorTopLeft, mgl.Vec3{x, -y, 0.0}, wString)
